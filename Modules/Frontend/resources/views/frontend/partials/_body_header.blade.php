...
<nav class="navbar navbar-expand-lg navbar-light px-0 py-3 header main-nav">
    <div class="container-xl ps-0 pe-0">
        ...
        <div class="collapse navbar-collapse ps-2" id="navbarCollapse">
            <div class="navbar-nav mx-lg-auto navbar-font">
                <a class="nav-item nav-link {{ request()->is('/') || request()->routeIs('browse') ? 'active' : '' }}" href="{{ route('browse') }}" aria-current="page">{{ __('frontend::message.home') }}</a>
                <a class="nav-item nav-link {{ request()->is('workout*') || (request()->routeIs('search') && (request()->get('section') == 'exercise') || request()->get('section') == 'bodypart_exercise') || request()->get('section') == 'exercise_level' || request()->get('section') == 'equipment_exercise'  ? 'active' : '' }}" href="{{ route('workouts') }}">{{ __('frontend::message.workouts') }}</a>
                <a class="nav-item nav-link {{ request()->is('diet*') || (request()->routeIs('search') && request()->get('section') == 'diet') ? 'active' : '' }}" href="{{ route('diet') }}">{{ __('frontend::message.diet') }}</a>
                <a class="nav-item nav-link {{ request()->is('product*') || (request()->routeIs('search') && request()->get('section') == 'product') ? 'active' : '' }}" href="{{ route('products') }}">{{ __('frontend::message.products') }}</a>
                <a class="nav-item nav-link {{ request()->is('blog*') || (request()->routeIs('search') && request()->get('section') == 'blog') ? 'active' : '' }}" href="{{ route('blog') }}">{{ __('frontend::message.blog') }}</a>
                @if(SettingData('subscription', 'subscription_system') == 1 && (!Auth::check() || !Auth::user()->is_subscribe))
                    <a class="nav-item nav-link {{ request()->routeIs(['pricing-plan', 'payment']) ? 'active' : '' }}" href="{{ route('pricing-plan') }}">{{ __('frontend::message.pricing') }}</a>
                @endif
                <a class="nav-item nav-link d-lg-none d-block" href="{{ route('language.setting') }}">{{ __('message.language_settings') }}</a>
            </div>
                    ...
                    <div class="card shadow-none m-0 border-0">
                        <div class="p-0">
                            <ul class="list-group list-group-flush">
                                @php
                                    $language_option = $app_settings->language_option;
            ...
            {{-- <div class="navbar-font mt-3 mt-md-0" id="search-toggle">
                <a href="#" class="text-decoration-none">Search<img src="{{ asset('frontend-section/images/magnifer.png') }}" class="ms-2"></a>
            </div> --}}
            ...
            <div class="mt-3 mt-lg-0 navbar-font">
                @if(Auth::check())
                    @if(Auth::user()->user_type == 'admin')
                        <a href="{{ route('dashboard') }}" class="nav-item nav-link">
                            ...
                            <span class="ms-1">{{ Auth()->user()->display_name }}</span>
                        </a>
                    @else
                        <a href="{{ route('user.dashboard') }}" class="nav-item nav-link">
                            <img src="{{ getSingleMedia(auth()->user(),'profile_image', null) }}" height="40" width="40" class="profile_image">
                            <span class="ms-1">{{ Auth()->user()->display_name }}</span>
                        </a>
                    @endif
                @else
                    {{-- <a href="" class="text-decoration-none">
                        <img src="{{ asset('frontend-section/images/user-rounded.png') }}" id="join_for_free" class="ms-lg-4">
                    </a> --}}
                    <a href="{{ route('frontend.signin') }}" class="text-decoration-none ms-2 pre-menu">{{ __('auth.login') }} |</a>
                    <a href="{{ route('signup') }}" class="text-decoration-none pre-menu">{{ __('frontend::message.register') }}</a>
                @endif
            </div>
        </div>
    </div>
