<x-frontend-auth-layout>
    <section>
        <div class="row m-0 align-items-center vh-100 carousel-section">
            ...
            <div class="col-lg-6 h-100 d-flex align-items-center login-section">
                <div class="row justify-content-center mb-4">
                    <div class="col-md-10">
                        <div class="border-0 mb-0 auth-card">
                            <div class="auth-cardbody">
                                <div class="mb-5">
                                    <h4 class="font-h4 heading">{{ __('auth.login') }}</h4>
                                    <span class="font-p">{{ __('frontend::message.welcome_back_please_enter_your_details') }}</span>
                                </div>
                         
                                <form method="POST" action="{{ route('frontend.signin') }}" data-toggle="validator" id="login_form">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="form-group">
                                                ...
                                                <input id="email" type="email" name="email" value="{{ old('email') }}" class="form-control form-control-data" required placeholder="{{ __('message.email') }}" autofocus>
                                            </div>
                                        </div>
                                        <div class="col-lg-12 mb-3">
                                            <div class="form-group">
                                                <label for="password" class="form-label font-p">{{ __('message.password') }}</label>
                                                <div class="password-container">
                                                    <input class="form-control form-control-data password password-input" type="password" placeholder="********" id="password" name="password" required autocomplete="current-password">
                                        ...
                                        <div class="d-flex justify-content-between mb-4">
                                            <div class="form-check">
                                                <input class="form-check-input customcheck" name="remember" type="checkbox" value="" id="customCheck1"/>
                                                <label class="form-check-label remember title-color" for="customCheck1">{{ __('auth.remember_me') }}</label>
                                            </div>
                                            <a href="{{ route('frontend.password.request') }}" class="forgot-password text-decoration-none">{{ __('auth.forgot_password') }}</a>
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-center mb-4">
                                        <button type="submit"
                                            class="btn login-btn text-white w-100 col-lg-12">{{ __('auth.login') }}</button>
                                    </div>
                                    <hr class="login-hr">
                                    <div class="row mt-4">
                                        <div class="col-6 col-md-6 mb-4">
                                            <a class="btn google-otp-btn w-100 d-flex align-items-center justify-content-center title-color border-dark" href="{{ route('otp-login') }}">
                                                <img id="phoneImage" src="{{ asset('frontend-section/images/phone-light.png') }}" class="me-2" alt="OTP Login">{{ __('frontend::message.otp_login') }}
                                            </a>
                                        </div>
                                        <div class="col-6 col-md-6 mb-4">
                                            <a class="btn google-otp-btn w-100 d-flex align-items-center justify-content-center title-color border-dark" href="{{ route('google.login') }}">
                                                <img id="googleImage" src="{{ asset('frontend-section/images/google-light.png') }}" class="me-2" alt="Google Login"> {{ __('frontend::message.google_login') }}
                                            </a>
                                        </div>
                                    </div>
                                    <p class="new-user text-center title-color">{{ __('frontend::message.new_user') }} <a href="{{ route('signup') }}" class="text-decoration-none"><span class="register-now">{{ __('frontend::message.registe_now') }}</span></a></p>
                                </form>
                            </div>
                        </div>
